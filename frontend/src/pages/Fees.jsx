    ...
    <DashboardLayout>
      <div className="mb-6">
        ...
        <p className="text-sm text-slate-500 mt-1">
          Fee structures, payments, and defaulters
        </p>
      </div>
...
function FeeStructuresTab() {
  ...
  const columns = [
    ...
    },
    {
      key: "installments",
      label: "Installments",
      render: (row) => row.installments?.length,
    },
  ];
              ...
              {formData.installments.map((inst, i) => (
                <div key={i} className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                  <input
                    value={inst.label}
                    onChange={(e) =>
                      updateInstallment(i, "label", e.target.value)
                    }
                    placeholder="Label"
      ...
      const options = {
        ...
        handler: async (response) => {
          try {
            ...
            setMessage({
              type: "success",
              text: `Payment verified. Receipt: ${res.data.receiptNumber}`,
            });
          } catch (err) {
            setMessage({
              type: "error",
              text: "Payment succeeded but verification failed — contact support",
            });
          }
        },
...
function DefaultersTab() {
  ...
  const columns = [
    ...
    },
    {
      key: "totalPaid",
      label: "Paid",
      render: (row) => `₹${row.totalPaid.toLocaleString()}`,
    },
    {
      key: "balanceDue",
      label: "Balance Due",
