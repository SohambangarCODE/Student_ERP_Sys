...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    getChildResults(selectedChild).then((res) => setResults(res.data)).finally(() => setLoading(false));
  }, [selectedChild]);
    ...
    <DashboardLayout>
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">View exam performance and marks</p>
        </div>
        {children.length > 1 && (
          <select
            value={selectedChild}
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {r.marks.map((m) => (
                    <div key={m.subjectName} className="bg-slate-50 rounded-lg p-3 text-center">
                      ...
                      <p className="text-sm font-semibold text-slate-900 mt-1">{m.marksObtained}</p>
                    </div>
                  ))}
                </div>
              </div>
