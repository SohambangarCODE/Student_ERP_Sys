      ...
      const options = {
        ...
        handler: async (response) => {
          try {
            ...
            setMessage({ type: 'error', text: 'Payment succeeded but recording it failed — contact your institute.' });
          }
        },
    ...
    <DashboardLayout>
      <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">View and pay outstanding fees</p>
        </div>
        {children.length > 1 && (
          <select
            value={selectedChild}
      ...
      {message && (
        <div className={`rounded-lg px-3 py-2 text-sm border mb-4 ${
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {structures.map((s) => (
            <div key={s._id} className="bg-white rounded-xl border border-slate-200 p-5 flex items-center justify-between flex-wrap gap-4">
              <div>
                <p className="text-sm text-slate-500">Total Fee</p>
                <p className="text-lg font-semibold text-slate-900">₹{s.totalAmount.toLocaleString()}</p>
                <p className="text-xs text-slate-400 mt-1">₹{s.totalPaid.toLocaleString()} paid so far</p>
              </div>
              <div className="text-right">
                <p className="text-sm text-slate-500">Balance Due</p>
                <p className={`text-lg font-semibold ${s.balanceDue > 0 ? 'text-red-600' : 'text-green-600'}`}>
                  ₹{s.balanceDue.toLocaleString()}
                </p>
              </div>
              {s.balanceDue > 0 ? (
                <Button onClick={() => handlePay(s)} loading={payingId === s._id}>Pay Now</Button>
              ) : (
